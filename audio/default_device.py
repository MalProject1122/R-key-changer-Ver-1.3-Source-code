...
def _build_interfaces() -> Any:
    ...
    class _IMMDeviceEnumerator(IUnknown):
        ...
        _methods_ = [
            ...
            # vtable の3番目。末尾に足すだけなので既存メソッドの位置は変わらない
            COMMETHOD(
                [], HRESULT, "GetDevice",
...
ROLES: tuple[int, ...] = (_ECONSOLE, _EMULTIMEDIA, _ECOMMUNICATIONS)
"""既定出力の3つの役割。設定ファイルには :data:`ROLE_KEYS` の名前で保存する。"""

ROLE_KEYS: dict[int, str] = {
    _ECONSOLE: "console",
    _EMULTIMEDIA: "multimedia",
    _ECOMMUNICATIONS: "communications",
}


def _create_enumerator() -> Any:
    import comtypes  # noqa: PLC0415
    import comtypes.client  # noqa: PLC0415
    ...
    return enumerator, interfaces


def get_default_outputs_by_role() -> dict[int, tuple[str, str | None]]:
    """3つの役割それぞれの既定出力を ``{役割: (デバイスID, 表示名)}`` で返す。

    デバイス名は同じ名前の機器が複数あると区別できないため、元へ戻す
    ときの主な手がかりには ``IMMDevice.GetId()`` の ID を使う。取得できない
    役割は結果に含めない（例外は投げない）。
    """
    if not is_available():
        return {}
    ...
    try:
        enumerator, interfaces = _create_enumerator()
    except Exception:  # noqa: BLE001
        logger.warning("既定の出力デバイスを取得できません", exc_info=True)
        return {}
    _, _, propstore_cls, key_cls, _, _ = interfaces
    for role in ROLES:
        try:
            device = enumerator.GetDefaultAudioEndpoint(_ERENDER, role)
            device_id = device.GetId()
            if device_id:
                result[role] = (str(device_id), _friendly_name(device, propstore_cls, key_cls))
        except Exception:  # noqa: BLE001 - 既定が無い役割もありうる
            logger.warning("既定の出力デバイスを取得できません (role=%s)", role, exc_info=True)
    return result


def set_default_output_by_id(device_id: str, role: int) -> bool:
    """ID で指定した出力デバイスを、指定した1つの役割の既定にする。

    デバイスが既に存在しない（USB 機器を抜いた等）・無効になっている
    場合は何もせず False を返す（その役割の復元だけを諦め、他の役割の
    復元は呼び出し側が続けられるようにするため）。
    """
    if not is_available() or not device_id:
        return False
    import comtypes  # noqa: PLC0415
    import comtypes.client  # noqa: PLC0415
    ...
    try:
        ...
        except Exception:  # noqa: BLE001 - 見つからない
            logger.warning("元の出力デバイスが見つかりません (role=%s, id=%s)", role, device_id)
            return False
        if state != _DEVICE_STATE_ACTIVE:
            logger.warning(
                "元の出力デバイスが使えない状態です (role=%s, id=%s, state=%s)", role, device_id, state
            )
            return False
        ...
        logger.warning("既定デバイスを元に戻せません (role=%s, id=%s)", role, device_id, exc_info=True)
        return False


def set_default_output_by_name(name: str, roles: tuple[int, ...] = ROLES) -> bool:
    """指定した名前の出力デバイスを既定にする。
