...
class AppSettings:
    ...
    original_output_ids: dict[str, str] = field(default_factory=dict)
    """起動前の既定出力デバイスID（``IMMDevice.GetId()``）を役割ごとに覚えたもの。
    キーは ``"console"`` / ``"multimedia"`` / ``"communications"``
    （:data:`audio.default_device.ROLE_KEYS`）。

    :attr:`last_real_output_name` と同じく、強制終了で既定が CABLE のまま
    残ったとき、次回起動時の復元に使う。名前は同名の機器を区別できず、
    また3役割が別々のデバイスを向いていることもあるため、こちらを優先する。
    CABLE 系を指していた役割は上書きしない。"""
    ...
    def from_dict(cls, data: dict[str, Any]) -> AppSettings:
        ...
        return cls(
            ...
            original_output_ids=_as_str_dict(data.get("original_output_ids")),
            live_space_params=(
...
def _as_float_list(value: Any, default: list[float]) -> list[float]:
    ...
    if not isinstance(value, list) or len(value) != len(default):
        return list(default)
    # json.loads は NaN / Infinity / -Infinity をそのまま float として通すため、
    # 有限値かどうかも確認する（NaN が EQ に入ると音声全体が NaN になる）
    if not all(
