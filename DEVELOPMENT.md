...
| Apple Music（Windows版）では加工されない（音は流れるが素通り） | 既定の出力を切り替えても Apple Music が追従せず、スピーカーへ直接出力していると推測（未確定）。「R-key changer を先に起動してから Apple Music を起動」「音量ミキサーで Apple Music の出力を CABLE Input に指定」で回避できるかは未検証 |
